...
describe('TodoListService', () => {
  ...
  describe('getAllItems', () => {
    it('should fetch all todo items', async () => {
      vi.mocked(apiClient.get).mockResolvedValue({ data: mockTodoItems });

      const result = await todoListService.getAllItems();

      expect(apiClient.get).toHaveBeenCalledWith('/api/v1/TodoList/items');
  ...
  describe('addItem', () => {
    ...
    it('should throw error when adding item fails', async () => {
      ...
      await expect(todoListService.addItem(newItem)).rejects.toThrow(
        'Validation error'
      );
    });
  });
  ...
  describe('updateItem', () => {
    ...
    it('should throw error when updating fails', async () => {
      ...
      await expect(todoListService.updateItem(999, updateData)).rejects.toThrow(
        'Item not found'
      );
    });
  });
  ...
  describe('deleteItem', () => {
    it('should delete a todo item', async () => {
      ...
      expect(apiClient.delete).toHaveBeenCalledWith('/api/v1/TodoList/items/1');
    });
    ...
    it('should throw error when deleting fails', async () => {
      ...
      await expect(todoListService.deleteItem(999)).rejects.toThrow(
        'Item not found'
      );
    });
  });
  ...
  describe('registerProgression', () => {
    it('should register a progression entry', async () => {
      ...
      expect(apiClient.post).toHaveBeenCalledWith(
        '/api/v1/TodoList/items/1/progressions',
        progressionData
      );
    });
    ...
    it('should throw error when registering progression fails', async () => {
      ...
      await expect(
        todoListService.registerProgression(1, progressionData)
      ).rejects.toThrow('Invalid progression data');
    });
  });
  ...
  describe('printItems', () => {
    it('should call print items endpoint', async () => {
      ...
      expect(apiClient.post).toHaveBeenCalledWith('/api/v1/TodoList/items/print');
    });
